...
public sealed class ProjectRepository(MarshalDbContext db, IKolejkaBazy? kolejka = null)
    : IProjectRepository
{
    ...
    public async Task<Project?> FindAsync(Guid id, CancellationToken ct = default) =>
        await _kolejka.WykonajAsync(() => db.Projects.FirstOrDefaultAsync(p => p.Id == id && !p.Deleted, ct), ct);

    public async Task<IReadOnlyList<Project>> ActiveAsync(CancellationToken ct = default) =>
        await _kolejka.WykonajAsync(() => db.Projects
