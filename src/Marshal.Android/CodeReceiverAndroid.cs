...
/// </para>
/// </remarks>
internal sealed class CodeReceiverAndroid(Context context) : ICodeReceiver
{
    /// <summary>
        ...
        AuthorizationCodeRequestUrl url, CancellationToken ct)
    {
        ...
        var listening = new TcpListener(IPAddress.Loopback, _port);
        listening.Start();
        ...
        await using var interrupt = clock.Token.Register(listening.Stop);
        ...
        SignInService.Watch(context, enable: true);
        ...
        try
        {
            ...
            Say($"Czekam na zgodę Google — 127.0.0.1:{_port}");

            var socket = Socket(listening, clock.Token);

            if (await Task.WhenAny(socket, manual) == manual)
            {
                ...
                _ = socket.ContinueWith(
                    task => _ = task.Exception, TaskScheduler.Default);

                return new AuthorizationCodeResponseUrl(Pola(await manual));
            }
        ...
        catch (System.OperationCanceledException) when (!ct.IsCancellationRequested)
        {
            ...
            throw new TimeoutException(
                $"Przeglądarka nie wróciła ze zgodą w ciągu {Patience.TotalMinutes:0} minut. "
        ...
        {
            SignInService.Watch(context, enable: false);
            ConsentReturn.Stop();
            listening.Stop();
        }
    }
    ...
    private async Task<AuthorizationCodeResponseUrl> Socket(
        TcpListener listening, CancellationToken ct)
    {
        while (true)
        {
            using var connection = await listening.AcceptTcpClientAsync(ct);
            ...
            {
                await Reply(stream, "Marshal czeka na zgodę.", ct);
                continue;
            }

            await Reply(stream, "Zgoda przyjęta. Możesz wrócić do Marshala.", ct);
    ...
    private void Say(string content) =>
        new Handler(Looper.MainLooper!).Post(
            () => Toast.MakeText(context, content, ToastLength.Long)?.Show());

    /// <summary>Port wybrany przez system. Zajęty na stałe byłby zajęty akurat wtedy, gdy trzeba.</summary>
    private static int FreePort()
    {
        var test = new TcpListener(IPAddress.Loopback, 0);
        test.Start();
        var port = ((IPEndPoint)test.LocalEndpoint).Port;
        test.Stop();
        return port;
    }
    ...
    {
        var intent = new Intent(Intent.ActionView, global::Android.Net.Uri.Parse(address.ToString()));
    ...
    private static async Task Reply(NetworkStream stream, string content, CancellationToken ct)
    {
