...
public sealed class RecurrenceScheduleTests
{
    ...
    public void Tygodniowo_idzie_do_najblizszego_wskazanego_dnia()
    {
        ...
        var regula = new RecurrenceRule(
            RecurrenceKind.Weekly, daysOfWeek: Weekdays.Monday | Weekdays.Thursday);

        RecurrenceSchedule.Next(regula, D("2026-09-16")).Should().Be(D("2026-09-17"));
    }

    [Fact]
    public void Tygodniowo_przechodzi_przez_koniec_tygodnia()
    {
        var regula = new RecurrenceRule(RecurrenceKind.Weekly, daysOfWeek: Weekdays.Monday);

        RecurrenceSchedule.Next(regula, D("2026-09-14")).Should().Be(D("2026-09-21"));
    }
    ...
    public void Co_drugi_tydzien_bierze_oba_dni_tego_samego_tygodnia_a_potem_przeskakuje()
    {
        ...
        var czwartek = RecurrenceSchedule.Next(regula, D("2026-09-14"));
        czwartek.Should().Be(D("2026-09-17"));

        RecurrenceSchedule.Next(regula, czwartek!.Value).Should().Be(D("2026-09-28"));
    }
    ...
    {
        var regula = new RecurrenceRule(RecurrenceKind.Monthly, dayOfMonth: 15);

        RecurrenceSchedule.Next(regula, D("2026-09-15")).Should().Be(D("2026-10-15"));
    }
    ...
    public void Miesiecznie_wypada_jeszcze_w_tym_miesiacu_gdy_dzien_dopiero_nadchodzi()
    {
        ...
        RecurrenceSchedule.Next(regula, D("2026-09-03")).Should().Be(D("2026-09-15"));
    }
    ...
    public void Trzydziesty_pierwszy_przycina_sie_do_dlugosci_miesiaca(string baza, string oczekiwana)
    {
        ...
        var regula = new RecurrenceRule(RecurrenceKind.Monthly, dayOfMonth: RecurrenceRule.LastDay);

        RecurrenceSchedule.Next(regula, D(baza)).Should().Be(D(oczekiwana));
    }
    ...
    public void Przyciecie_nie_zjada_dnia_na_stale()
    {
        ...
        RecurrenceSchedule.Next(regula, D("2026-02-28")).Should().Be(D("2026-03-31"));
    }

    [Fact]
    public void Dwudziesty_dziewiaty_lutego_w_roku_zwyklym_schodzi_na_dwudziesty_osmy()
    {
        var regula = new RecurrenceRule(RecurrenceKind.Yearly);

        RecurrenceSchedule.Next(regula, D("2028-02-29")).Should().Be(D("2029-02-28"));
    }
    ...
    {
        var regula = new RecurrenceRule(RecurrenceKind.Daily, until: D("2026-09-17"));

        RecurrenceSchedule.Next(regula, D("2026-09-16")).Should().Be(D("2026-09-17"));
        RecurrenceSchedule.Next(regula, D("2026-09-17")).Should().BeNull();
    }

    [Fact]
    public void Ostatnie_wystapienie_nie_ma_nastepnika()
    {
        var regula = new RecurrenceRule(RecurrenceKind.Daily, count: 1);

        RecurrenceSchedule.Next(regula, D("2026-09-16")).Should().BeNull();
    }

    [Fact]
    public void Licznik_wystapien_maleje_o_jeden_na_wystapienie()
    {
        var regula = new RecurrenceRule(RecurrenceKind.Daily, count: 3);

        regula.Advance().Count.Should().Be(2);
        regula.Advance().Advance().Count.Should().Be(1);
        RecurrenceSchedule.Next(regula.Advance().Advance(), D("2026-09-16")).Should().BeNull();
    }

    [Fact]
    public void Seria_bez_konca_zostaje_bez_konca()
    {
        var regula = new RecurrenceRule(RecurrenceKind.Daily);

        regula.Advance().Count.Should().BeNull();
    }
    ...
    public void Domyslne_zaczepienie_wynika_z_rodzaju(RecurrenceKind rodzaj, RecurrenceAnchor oczekiwane)
    {
        RecurrenceRule.DefaultAnchorFor(rodzaj).Should().Be(oczekiwane);
    }
    ...
    public void Zmiana_czasu_nie_przesuwa_dnia()
    {
        ...
        RecurrenceSchedule.Next(regula, D("2026-10-19")).Should().Be(D("2026-10-26"));
    }
    ...
    public void Regula_przechodzi_przez_zapis_i_odczyt_bez_zmian()
    {
        ...
        RecurrenceRule.FromJson(regula.ToJson()).Should().Be(regula);
    }
